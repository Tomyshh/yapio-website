    ...
    <section className="testimonials-stack relative">
      {testimonials.map((testimonial, index) => {
        ...
        if (displayOrder === 0) { // The active card
          style.transform = `${baseTransform} translateX(${dragOffset}px)`;
          style.opacity = 1;
          ...
          style.transform = `${baseTransform} scale(${scale}) translateY(${translateY}rem)`;
          style.opacity = 1 - 0.2 * displayOrder;
          style.zIndex = totalCards - displayOrder;
        } else { // Cards that are out of view
          style.transform = `${baseTransform} scale(0)`;
          style.opacity = 0;
          style.zIndex = 0;
        }
